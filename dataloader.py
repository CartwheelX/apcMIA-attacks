...
# this one has been used for all exp
def prepare_dataset(dataset_name, attr, root, device, arch, DSize):
    ...
    length = len(dataset)
    # length = math.floor(length/2) # need to remove after, its just for SGD-DP
...
# def prepare_dataset(dataset_name, attr, root, device, arch, DSize=None):
#     """
#     DSize controls the total subset size *before* the 50/50 stratified split.
#       - None or "all": use full dataset
#       - float in (0,1]: use that fraction of the dataset
#       - int >= 2*num_classes (recommended): use that many samples

#     Returns:
#         num_classes, target_train (Subset), target_test (Subset), target_model
#     """
...
#     # --- Get labels efficiently when possible ---
#     labels = None
...
#     if labels is None:
#         # Fallback: iterate (may be slower for large image datasets)
#         labels_list = []
...
#     # --- Resolve desired subset size (N_sel) ---
#     if DSize is None or DSize == "all":
#         N_sel = length
#     elif isinstance(DSize, float) and 0 < DSize <= 1:
#         N_sel = int(round(length * DSize))
#         N_sel = max(N_sel, 2 * num_classes)  # ensure minimally viable size
#         N_sel = min(N_sel, length)
#     elif isinstance(DSize, int) and DSize > 0:
#         N_sel = min(DSize, length)
#     else:
#         raise ValueError("DSize must be None/'all', a float in (0,1], or a positive int.")

#     # --- Quick viability check for stratification ---
#     # Need at least 2 samples of the minority class in the *selected subset*
#     class_counts = np.bincount(labels, minlength=num_classes)
#     min_prop = class_counts.min() / length
#     if int(np.floor(N_sel * min_prop)) < 2:
#         min_needed = int(np.ceil(2.0 / max(min_prop, 1e-12)))
#         raise ValueError(
#             f"DSize={DSize} is too small for a stratified 50/50 split "
#             f"given the minority class proportion. Use at least {min_needed} samples "
#             f"(or increase the fraction) so each class has >=2 samples before splitting."
#         )

#     # --- Stratified downsample to N_sel (if needed) ---
#     if N_sel < length:
...
#             random_state=42,
#         )
#         labels_sel = labels[selected_idx]
#     else:
#         selected_idx = indices
#         labels_sel = labels

#     # --- 50/50 stratified split on the selected subset ---
#     train_idx, test_idx = train_test_split(
#         selected_idx,
#         train_size=0.5,
#         stratify=labels_sel,
...
#     # Helpful logs
#     def dist_str(arr_labels):
#         c = Counter(arr_labels.tolist())
#         parts = [f"{k}:{v}" for k, v in sorted(c.items())]
#         return "{" + ", ".join(parts) + "}"

#     print(f"Dataset {dataset_name}: total {length} samples; using {len(selected_idx)} for experiment.")
#     print(f" Train: {len(train_idx)}  Test: {len(test_idx)}")
#     print(f" Class dist (train): {dist_str(labels[train_idx])}")
#     print(f" Class dist (test) : {dist_str(labels[test_idx])}")
...
#     # train_sub    indices = np.arange(length)
    # labels = []
    # for idx in indices:
    #     _, target = dataset[idx]
    #     if isinstance(target, (list, tuple)):
...
#     target_train = torch.utils.data.Subset(dataset, train_idx.tolist())
#     target_test = torch.utils.data.Subset(dataset, test_idx.tolist())
#     t size: {len(target_test)} samples testing")
# #     # print(f"prepare_dataset-Type of target_train: {type(target_train)}")
# #     # exit()
# #     return num_classes, target_train, target_test, target_model
...
def get_model_dataset(dataset_name, device, arch, attr, root):
    ...
    return num_classes, dataset, target_model, target_model
